...
<script>
    ...
    // '.box' というクラスを持つ要素をすべて取得
    const boxes = document.querySelectorAll('.box');

    let activeBox = null; // 現在つかんでいるボックス
    let offsetX = 0;
    let offsetY = 0;

    // すべてのボックスに対して「マウスを押したとき」のイベントを設定
    boxes.forEach(box => {{
        box.addEventListener('mousedown', (e) => {{
            activeBox = box; // どのボックスがつかまれたかを覚えておく
            offsetX = e.clientX - box.offsetLeft;
            offsetY = e.clientY - box.offsetTop;
        }});
    }});

    // マウスを動かしたとき（つかんでいるボックスがあれば移動）
    document.addEventListener('mousemove', (e) => {{
        if (!activeBox) return; // 何もつかんでいなければ何もしない

        let x = e.clientX - offsetX;
        let y = e.clientY - offsetY;

        // 枠外にはみ出さない計算
        const maxX = canvas.clientWidth - activeBox.clientWidth;
        const maxY = canvas.clientHeight - activeBox.clientHeight;
